...
| Product interview | Jakub | UI | `POST /api/interviews/respond` |
| Voice session | Jakub | UI | `POST /api/realtime/token` + WebRTC |
| Product search | Jakub | UI | `POST /api/products/search` |
| Live offer scraping | Jakub | UI | `POST /api/offers/scrape` |
